...
public:
    ...
    virtual int32_t InitEmpty() { return -1; }
    virtual int32_t UpdateBaseMeta(const uint8_t *buffer, int32_t len)
    {
        (void)buffer;
        (void)len;
        return -1;
    }
    virtual int32_t AddObject(int32_t &objectIndex)
    {
        (void)objectIndex;
        return -1;
    }
    virtual int32_t RemoveObject(int32_t objectIndex)
    {
        (void)objectIndex;
        return -1;
    }
