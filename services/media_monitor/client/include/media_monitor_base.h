...
public:
    ...
    *
    * @param preferredDevices MonitorDeviceInfo map
    */
    virtual int32_t GetAudioRouteMsg(std::map<PreferredType,
        std::shared_ptr<MonitorDeviceInfo>> &preferredDevices) = 0;
    ...
    virtual int32_t GetPcmDumpStatus(int32_t &dumpEnable) = 0;

    virtual int32_t ErasePreferredDeviceByType(const PreferredType preferredType) = 0;

    virtual int32_t GetAudioExcludedDevicesMsg(std::map<AudioDeviceUsage,
        std::vector<std::shared_ptr<MonitorDeviceInfo>>> &excludedDevices) = 0;
